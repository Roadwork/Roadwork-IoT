...
using IRoadworkTwinActorInterface;
...
namespace RoadworkTwinActor
{
    ...
    /// </summary>
    public class RoadworkTwinActor : Actor, IRoadworkTwinActor, IRemindable
    {
        private const string StateName = "my_data";

        /// <summary>
        /// Initializes a new instance of the <see cref="RoadworkTwinActor"/> class.
        /// </summary>
        /// <param name="service">Actor Service hosting the actor.</param>
        /// <param name="actorId">Actor Id.</param>
        public RoadworkTwinActor(ActorService service, ActorId actorId)
            : base(service, actorId)
        {
        }

        /// <inheritdoc/>
        public async Task SaveData(RoadworkTwin data)
        {
            ...
            await this.StateManager.SetStateAsync<RoadworkTwin>(StateName, data);
        }

        /// <inheritdoc/>
        public Task<RoadworkTwin> GetData()
        {
            // Get state using StateManager.
            return this.StateManager.GetStateAsync<RoadworkTwin>(StateName);
        }
        ...
        public Task ReceiveReminderAsync(string reminderName, byte[] state, TimeSpan dueTime, TimeSpan period)
        {
            ...
            var actorState = this.StateManager.GetStateAsync<RoadworkTwin>(StateName).GetAwaiter().GetResult();
            this.StateManager.SetStateAsync<RoadworkTwin>(StateName, actorState);
            return Task.CompletedTask;
        }
        ...
        // {
        //     var state = this.StateManager.GetStateAsync<RoadworkTwin>(StateName).GetAwaiter().GetResult();
        //     state.PropertyA = $"Timer triggered at '{DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss")}'";
        //     this.StateManager.SetStateAsync<RoadworkTwin>(StateName, state);
        //     return Task.CompletedTask;
        // }
    }
}